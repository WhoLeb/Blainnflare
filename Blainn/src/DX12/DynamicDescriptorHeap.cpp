...
namespace Blainn
{
DynamicDescriptorHeap::DynamicDescriptorHeap( Microsoft::WRL::ComPtr<ID3D12Device>& device, D3D12_DESCRIPTOR_HEAP_TYPE heapType,
...
void DynamicDescriptorHeap::ParseRootSignature( const std::shared_ptr<dx12lib::RootSignature>& rootSignature )
{
    ...
    std::function<void( ID3D12GraphicsCommandList*, UINT, D3D12_GPU_DESCRIPTOR_HANDLE )> setFunc )
{
    ...
    {
        auto d3d12Device              = m_Device;
        auto d3d12GraphicsCommandList = commandList;
        ...
        while ( _BitScanForward( &rootIndex, m_StaleDescriptorTableBitMask ) )
        {
            ...
            setFunc( d3d12GraphicsCommandList.Get(), rootIndex, m_CurrentGPUDescriptorHandle );
...
void DynamicDescriptorHeap::CommitStagedDescriptorsForDraw(
    Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList>& commandList )
{
    CommitDescriptorTables( commandList,
...
//{
//    CommitDescriptorTables( commandList.GetD3D12CommandList(),
                                                                   ...
                                                                   D3D12_CPU_DESCRIPTOR_HANDLE cpuDescriptor )
{
    if ( !m_CurrentDescriptorHeap || m_NumFreeHandles < 1 )
    {
        ...
        Application::Get().GetResourceManager()->SetDescriptorHeap(
            comandList.GetD3D12CommandList(),
            m_DescriptorHeapType,
            m_CurrentDescriptorHeap.Get()
        );
